...
export default function ColorPicker({ isEditing }: { isEditing?: boolean }) {
  ...
  // Check URL param ?colorpicker=1
  const urlParams = new URLSearchParams(window.location.search);
  const forceShow = urlParams.get('colorpicker') === '1';
  ...
  // Only show in edit mode OR when ?colorpicker=1 is in URL
  if (!isEditing && !forceShow) return null;

  return (
    <div ref={panelRef} className="fixed top-[60px] right-[5px] z-[9998] font-['Inter',sans-serif]">
      {open && (
        <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 bg-white rounded-xl shadow-[-8px_8px_30px_rgba(0,0,0,0.35)] border border-gray-100 overflow-hidden animate-fade-in-scale">
          {/* Header */}
          ...
          {/* Font accent picker — edit mode only */}
          {isEditing && (
